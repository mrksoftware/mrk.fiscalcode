...
namespace mrk.fiscalcode
{
    public class CodiceFiscale
    {
        ...
        {
            var assembly = Assembly.GetAssembly(typeof(mrk.fiscalcode.CodiceFiscale));
            var resource = assembly.GetManifestResourceStream("mrk.fiscalcode.Assets.data.xml");

            data = new DataSet();
            data.ReadXml(resource);
            omocodiciRegex = @"[0-9";
